...
The justfile has the rest: the frontend checks, the runnable examples, and the live-Azure
...
The integration tests need credentials for the maintainer's Azure test account, so most contributors
cannot run them locally. That is fine. A PR from a fork is not given the secret either, so that job
skips there rather than failing on empty credentials, and the run against live Azure happens after
merge. If your change touches the Azure backend, say so in the PR and I will run them. The azure
fixtures in `tests/conftest.py` have the details.
